 ...
 *    each transaction as :tx / :rx event entries (it appends those segments
 *    itself), so a duplex endpoint registers with no unit.
 */
...
typedef struct
{
    ...
    // Register a signal <sigType>:<localName> of the given kind, with optional
    // unit. Returns a handle, or a negative value on failure. The framework
    // prefixes its own member instance name — the driver never knows which
    // instance it is (two boards may run the same image).
...
// Register a scalar port. unit is optional (NULL for none). Null-safe (see the
// header comment).
int32_t SIL_ports_register(const char * const sigType, const char * const localName,
                           const char * const unit);

// Register a duplex transaction endpoint (no unit — the framework appends
// :tx/:rx on the record entries itself). Null-safe.
