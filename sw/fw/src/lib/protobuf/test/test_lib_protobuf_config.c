...
static void test_ping_roundtrip(void)
{
    ...
    env.request_id = 7U;
    env.which_payload = shared_Envelope_ping_tag;

    uint8_t buffer[LIB_PROTOBUF_ENVELOPE_MAX];
    size_t encodedLen = 0U;
    TEST_ASSERT_TRUE(lib_protobuf_encode(shared_Envelope_fields, &env, buffer, sizeof(buffer), &encodedLen));

    shared_Envelope decoded = shared_Envelope_init_zero;
    TEST_ASSERT_TRUE(lib_protobuf_decode(shared_Envelope_fields, buffer, encodedLen, &decoded));
    TEST_ASSERT_EQUAL_UINT32(7U, decoded.request_id);
    TEST_ASSERT_EQUAL(shared_Envelope_ping_tag, decoded.which_payload);
}
...
static void test_response_roundtrip(void)
{
    ...
    TEST_ASSERT_EQUAL(shared_Envelope_response_tag, decoded.which_payload);
...
static void test_log_roundtrip(void)
{
    ...
    TEST_ASSERT_EQUAL(shared_Envelope_log_tag, decoded.which_payload);
...
static void test_truncated_encoding_fails_decode(void)
{
    ...
    shared_Envelope decoded = shared_Envelope_init_zero;
    TEST_ASSERT_FALSE(lib_protobuf_decode(shared_Envelope_fields, buffer, encodedLen - 2U, &decoded));
}
