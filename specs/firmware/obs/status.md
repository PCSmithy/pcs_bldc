...
Periodic device status to the host: a `Telemetry` message assembled
from the motor-control snapshot and the bus measurements.
...
(`fw~est_velocity_001~1`), the speed target, the bus voltage, the
...
- A mode change, drive-state change, or speed-target change appears in
  the next published `Telemetry`.
