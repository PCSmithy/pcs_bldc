...
value — the drive mode (`sys~mc_005~1`) or the speed setpoint — the
command most recently arriving at the motor-control application shall
determine it.

Acceptance:

- A host setpoint command following a dial adjustment leaves the host's
  value in effect, and a further dial adjustment leaves the dial's.
