...
2 ms of any phase current magnitude exceeding 2 A or the VBUS current
exceeding 1.5 A, holding the bridge disabled until an explicit user action
clears the fault.

Acceptance:
- A phase current magnitude exceeding 2 A disables the bridge and latches
  the fault within 2 ms.
- A VBUS current exceeding 1.5 A disables the bridge and latches the fault
  within 2 ms.
