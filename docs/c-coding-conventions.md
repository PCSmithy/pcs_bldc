...
A dual-target module has exactly one public header,
...
Single-instance modules with no config at all (`HW_USB`) skip the
target header entirely — one root header, nothing else.
